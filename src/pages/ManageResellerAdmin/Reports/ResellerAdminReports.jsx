...
const ResellerAdminReports = () => {
  const dispatch = useDispatch();

  const [merchantId, setMerchantId] = useState("");
  const [resellerId, setResellerId] = useState("");
  const [resellerAdminId, setResellerAdminId] = useState("");

  /* -------------------------SELECTORS ------------------------ */

  const { resellerAdmins, resellers } = useSelector(
    (slice) => slice.reselleradmin
  );
  const { dashboardData } = useSelector((slice) => slice.dashboard);
  const { resellersMerchant } = useSelector((slice) => slice.reseller);
  ...
  useEffect(() => {
    dispatch(populateResellerAdmins_Reseller({ resellerAdminId }));
  }, [resellerAdminId]);
    ...
    <div className="flex gap-8 flex-col min-h-screen">
      <div className="flex items-center justify-between gap-8 mt-4">
        <select
          id="merchant"
          name="merchant"
          onChange={(e) => setResellerAdminId(e.target.value)}
...
export default ResellerAdminReports;
